...
describe "Anagram" do
  describe ".find" do
    ...
    it "detects multiple anagrams" do
      Anagram.find("master", ["stream", "pigeon", "maters"]).should eq ["maters", "stream"]
    end

    it "detects multiple anagrams 2" do
      Anagram.find("allergy", ["gallery", "ballerina", "regally", "clergy", "largely", "leading"]).should eq ["largely", "regally", "gallery"]
    end
